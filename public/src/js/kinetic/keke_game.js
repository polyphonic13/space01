...
function checkInput() {
	if(controls.getWasPressed('quitButton')) {
	    ...
	    if (keys[ControlKeys.LEFT] || controls.getReverse()) {
			...
			keke.direction = PWG.Directions.LEFT
			...
			keke.direction = PWG.Directions.RIGHT;
	...
	for(var i = 0; i < grounds.length; i++) {
		...
		if(rect.x < (kekeHitArea.x + kekeHitArea.width + 100) && rect.x > -((rect.width/2) + stageConfig.width)) { 
			...
			if(col.direction !== '') {
			    ...
			    if (col.direction === PWG.Directions.LEFT || col.direction === PWG.Directions.RIGHT) {
			        keke.velX = 0;
			        keke.jumping = false;
				} else if (col.direction === PWG.Directions.BOTTOM) {
					if(keke.justJumped) {
				...
				} else if (col.direction === PWG.Directions.TOP) {
			        // keke.velY *= -1;
			    }
		...
		if(enemyObjs[key].alive) {
			...
			if(enemyPos.x < (kekeHitArea.x + 100) && enemyPos.x > -(kekeHitArea.x + 100)) { 
				...
				if(col.direction !== '') {
					...
					if(col.direction === PWG.Directions.TOP && (kekeBottom < enemyVerticalCenter)) {
					...
					} else if(col.direction === PWG.Directions.LEFT || col.direction === PWG.Directions.RIGHT || col.direction === PWG.Directions.BOTTOM) {
						...
						if(keke.direction === PWG.Directions.RIGHT) {
							keke.velX++;
						} else {
							keke.velX--;
						}
...
function horizontalCollisionCheck(shapeA, shapeB) {
    ...
    if (Math.abs(col.vX) < col.hWidths && Math.abs(col.vY) < col.hHeights) {         // figures out on which side we are colliding (top, bottom, left, or right)         
		...
		if (oX >= oY) {
            if (col.vY > 0) {
                col.direction = PWG.Directions.TOP;
                shapeA.y += oY;
				col.id = shapeB.id;
            } else {
                col.direction = PWG.Directions.BOTTOM;
                shapeA.y -= oY;
				col.id = shapeB.id;
            }
		}
...
function verticalCollisionCheck(shapeA, shapeB) {
    ...
    if (Math.abs(col.vX) < col.hWidths && Math.abs(col.vY) < col.hHeights) {         // figures out on which side we are colliding (top, bottom, left, or right)         
		...
		if (oX >= oY) {
            if (col.vX > 0) {
                col.direction = PWG.Directions.LEFT;
                shapeA.x += oX;
				col.id = shapeB.id;
            } else {
                col.direction = PWG.Directions.RIGHT;
                shapeA.x -= oX;
				col.id = shapeB.id;
            }
        }
